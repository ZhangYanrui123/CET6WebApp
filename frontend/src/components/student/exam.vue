  ...
  export default {
    ...
    methods: {
      ...
      getExamData() { //获取当前试卷所有信息
        ...
        this.$axios({
            ...
            data: {
                ...
                let now = new Date()
            console.log(now)
                console.log(this.examData.eend)
      ...
      commit() {
        ...
        for(var i = 0; i < this.questions.length; i++){
            ...
            if(this.isChoice(this.questions[i].qtype)&&this.questions[i].qnum==0){
                if(this.answer[i] == 0){
                    ...
                    this.objectAnswer = this.objectAnswer == null? String.fromCharCode(64+this.answer[i]) : this.objectAnswer + "&" + String.fromCharCode(64+this.answer[i])
                }
            }
            //获取主观题答案
            else if(!this.isChoice(this.questions[i].qtype)&&this.questions[i].qnum==0){
                console.log('subjective')
                this.$axios({url: 'http://127.0.0.1:8081/api/pushr',
                    method: 'post',  //传输主观题答案
                data: {
                    uuid: this.user.uuid,
                    eid: this.eid,
        ...
        this.$axios({url: 'http://127.0.0.1:8081/api/pusha', method: 'post',  //传输客观题答案字符串
                    data: {
                        uuid: this.user.uuid,
                        eid: this.eid,
                        pid: this.pid,
